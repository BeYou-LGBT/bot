...
class ModerationVerification {
    ...
    constructor(moderator, bandit, msg) {
        ...
        this.bandit = bandit
        this.message = msg
        if (moderator.id == bandit.id) return this.message.client.channels.get(this.message.channel.id).send("Vous ne pouvez pas vous sanctionner vous-même")
        if(this.moderator.roles.has(config.roles.staff) == false || this.bandit.roles.has(config.roles.staff) == false) return this.message.channel.send("Vous n'avez pas la permission de sanctionner cette personne")
     }

    isValid(sanction_type, cmd_syntax) {
        args = this.message.content.split(" ")
    ...
    sendMessage(sanction_msg) {
        let embed = new RichEmbed().setAuthor(`L'utilisateur ${this.bandit.user.username}#${this.bandit.user.discriminator} a été ${sanction_msg}`)
        ...
        this.message.client.channels.get(config.channels.log).send(embed)
